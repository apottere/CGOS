...
void set_gui(){
	...
	for(i = 0; i < C; i++) {
		for(j = 0; j < R; j++) {
			gtk_image_set_from_pixbuf(GTK_IMAGE(FIELD[i][j]), IMAGE[CELLS[i][j]->isAlive]);
		}
	}
}
...
GtkWidget* make_gui() {
	...
	message_label = gtk_label_new("Starting countdown...");
	...
	for(i = 0; i <  C; i++) {
		FIELD[i] = malloc(sizeof(GtkWidget*)*R);
			for(j = 0; j <  R; j++) {
				FIELD[i][j] = gtk_image_new_from_pixbuf(IMAGE[CELLS[i][j]->isAlive]);
